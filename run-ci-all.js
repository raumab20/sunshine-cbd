...
await new Promise(resolve => setTimeout(resolve, 1000)); // 1 Sekunde warten und
}
}
...
while (attempts < 5) {
try {
execSync("NODE_OPTIONS='--max-old-space-size=1536' TURBO_FORCE=1 NEXT_DISABLE_CACHE=1 NEXT_STATIC_EXPORT=1 npm run build --no-lint --no-check", { stdio:
"inherit" });
console.log("✅ Build successful!");
break;
} catch (error) {
attempts++;
console.error(`❌ Build failed (attempt ${attempts}/5), retrying in 5
seconds...`);
...
console.log("🧪 Running all Cypress tests... SKIPPING FOR NOW");
...
console.log("⚠ SIGTERM nicht erfolgreich, erzwinge SIGKILL...");
