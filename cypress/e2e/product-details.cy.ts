describe("Product Details Page Tests", () => {
  ...
  before(() => {
    cy.request("/api/products").then((response) => {
      ...
      firstProduct = allProducts.find(
        (prod: any) => prod.category === uniqueCategory
      );
  ...
  it("should display related products or all products if no related products exist (maximum 10)", () => {
    cy.request(`/api/products?category=${firstProduct.category}`).then(
      (relatedResponse) => {
        const relatedProducts = relatedResponse.body.filter(
          (product: { id: any }) => product.id !== firstProduct.id
        );
        cy.log(
          `API: Es wurden ${relatedProducts.length} verwandte Produkte gefunden.`
        );

        let expectedCount;
        if (relatedProducts.length > 0) {
          expectedCount = Math.min(relatedProducts.length, 10);

          relatedProducts.forEach((product: any) => {
            expect(product.category).to.equal(firstProduct.category);
          });
        } else {
          const allProductsExceptCurrent = allProducts.filter(
            (product: { id: any }) => product.id !== firstProduct.id
          );
          expectedCount = Math.min(allProductsExceptCurrent.length, 10);
          cy.log(
            `Fallback: Es werden alle anderen (${allProductsExceptCurrent.length}) Produkte (max. 10) angezeigt.`
          );
        }

        cy.log(`Erwartete Anzahl angezeigter Produkte: ${expectedCount}`);
  ...
  it("should not include the current product in the related products list", () => {
    ...
    cy.get(
      '[data-testid="related-products"] a[href="/products/' +
        firstProduct.id +
        '"]'
    ).should("not.exist");
  });
  ...
  it("should navigate to a related product and ensure it shares the same category", () => {
    ...
    const multiProductCategory = Object.keys(categoryCounts).find(
      (cat) => categoryCounts[cat] > 1
    );
    ...
    const productWithRelated = allProducts.find(
      (prod: any) => prod.category === multiProductCategory
    );
    ...
    cy.request(`/api/products?category=${productWithRelated.category}`).then(
      (relatedResponse) => {
        const relatedProducts = relatedResponse.body;
        expect(relatedProducts.length).to.be.greaterThan(
          0,
          "Es sollten verwandte Produkte vorhanden sein"
        );
        ...
        cy.get('[data-testid="related-products"]')
          ...
          .then((href) => {
            ...
            cy.request(href!.replace("/products", "/api/products")).then(
              (res) => {
                const visitedProduct = res.body;
                expect(visitedProduct.category).to.equal(
                  productWithRelated.category
                );
              }
            );
          });
